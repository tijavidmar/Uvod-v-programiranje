...
#     'Blaž'
# =============================================================================
def izlocen(seznam, ime, dolzina):
    for i in range(len(seznam)):
        if seznam[i] == ime:
            zacetek = i
    izlocen_indeks = (zacetek + dolzina - 1) % len(seznam)
    return seznam[izlocen_indeks]
# =====================================================================@040228=
# 2. podnaloga
...
#     'Cilka'
# =============================================================================
def zmagovalec(igralci, prvi, dolzine):
    izstevanka = 0
    while len(igralci) > 1:
        zadnji = izlocen(igralci, prvi, dolzine[izstevanka])
        indeks = igralci.index(zadnji)
        igralci.pop(indeks)
        prvi = igralci[indeks % len(igralci)]
        izstevanka = (izstevanka + 1) % len(dolzine)
    return igralci[0]
# =====================================================================@040229=
# 3. podnaloga
...
#     'Cilka'
# =============================================================================
def pri_kom_zaceti(igralci, favorit, dolzine):
    for prvi in igralci:
        zmagal = zmagovalec(igralci.copy(), prvi, dolzine)
        if zmagal == favorit:
            return prvi
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDIyNywidXNlciI6MTE0MjJ9:1wWE7D:zFAsuJtbi1kxxtyOivPEbMQo7dL76RV_cxAVFxgbVjQ"
        try:
            testi = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDIyOCwidXNlciI6MTE0MjJ9:1wWE7D:IOnPUV8q3dFK6cq7bdojOL0Vfi3tdmuxhadd2gxs5RA"
        try:
            testi = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDIyOSwidXNlciI6MTE0MjJ9:1wWE7D:1FrzZI--RRzmYTJZl6hdZramBcl5yJmTew3pMZNOSco"
        try:
            testi = [
