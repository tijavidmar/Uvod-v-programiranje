...
def obrat(n):
    ...
    desetice = n // 10 % 10
    stotice = n // 100
    return 100 * enice + 10 * desetice + stotice 
# =====================================================================@000926=
# 3. podnaloga
...
def dodaj_kontrolno_stevko(sklic):
    ...
    kontrolna = 11 - (s1 * 2 + s2 * 3 + s3 * 4 + s4 * 5 + s5 * 6 + s6 * 7 +s7 * 8 + s8 * 9 + s9 * 10 + s10 * 11 + s11 * 12 + s12 * 13) % 11
    if kontrolna < 10:
        kontrolna = kontrolna
    else:
        kontrolna = 0
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjQsInVzZXIiOjExNDIyfQ:1vusMI:pGin8Th_Zpak1aS2dNkBDRPDOusOe87w94WCEp1x0AM"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjUsInVzZXIiOjExNDIyfQ:1vusMI:Xg9Mrf_cIYnE24A4ssOtgG046CmTzg3AJjUBZQxsF3k"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjYsInVzZXIiOjExNDIyfQ:1vusMI:CDwDEQwMxIUqqA-up0hDRDDeuZtqy6UYzo-uX5mEKe0"
        try:
